...
class OAuthService extends \WPForms\Vendor\Stripe\Service\AbstractService
{
    /**
    ...
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = \WPForms\Vendor\Stripe\Util\Util::encodeParameters($params);
    ...
    private function _parseOpts($opts)
    {
        if (\is_array($opts)) {
            if (\array_key_exists('connect_base', $opts)) {
                ...
                throw new \WPForms\Vendor\Stripe\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }
        return \WPForms\Vendor\Stripe\Util\RequestOptions::parse($opts);
    }
    /**
