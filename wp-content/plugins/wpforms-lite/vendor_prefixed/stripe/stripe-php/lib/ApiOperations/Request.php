...
trait Request
{
    ...
    protected static function _validateParams($params = null)
    {
        if ($params && !\is_array($params)) {
            ...
            throw new \WPForms\Vendor\Stripe\Exception\InvalidArgumentException($message);
        }
    }
    /**
    ...
    {
        $opts = \WPForms\Vendor\Stripe\Util\RequestOptions::parse($options);
        $baseUrl = isset($opts->apiBase) ? $opts->apiBase : static::baseUrl();
        $requestor = new \WPForms\Vendor\Stripe\ApiRequestor($opts->apiKey, $baseUrl);
