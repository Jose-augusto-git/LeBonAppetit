...
use WPForms\Vendor\Stripe\StripeObject;
abstract class Util
{
    ...
    {
        $types = \WPForms\Vendor\Stripe\Util\ObjectTypes::mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object']) && isset($types[$resp['object']])) {
                ...
                $class = \WPForms\Vendor\Stripe\StripeObject::class;
            }
    ...
    {
        if ($h instanceof \WPForms\Vendor\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
